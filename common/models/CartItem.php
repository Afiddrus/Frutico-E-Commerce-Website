...
class CartItem extends \yii\db\ActiveRecord
{
    ...
    public static function getTotalPriceForUser($currUserId)
    {
        if (isGuest()) {
            ...
            $sum =  CartItem::findBySql(
                "SELECT SUM(c.quantity * p.price) FROM cart_items c  LEFT JOIN  products p  on p.id = c.product_id WHERE c.created_by = :userId",
                ['userId' => $currUserId]
            )
                ->scalar();
        }
    ...
    public static function clearCart(int $currUserId)
    {
        if (isGuest()) {
            // If user is a guest, clear cart items from session
            \Yii::$app->session->remove(self::SESSION_KEY);
        } else {
            // If user is authenticated, delete cart items from database
